...
def patch_warp(H, uv):
    ...
    grid = grid_tmp[..., :2] / (grid_tmp[..., 2:] + 1e-7)
    return grid
...
def render_normal_func(viewpoint_cam, depth, offset=None, normal=None, scale=1):
    ...
    # normal_ref: (3, H, W)
    intrinsic_matrix = torch.tensor([[viewpoint_cam.Fx/scale, 0, viewpoint_cam.Cx/scale], [0, viewpoint_cam.Fy/scale, viewpoint_cam.Cy/scale], [0, 0, 1]]).float()
